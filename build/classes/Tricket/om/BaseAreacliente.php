...
/**
 * Base class that represents a row from the 'AreaCliente' table.
 *
 * 
 *
 * @package    propel.generator.Tricket.om
 */
abstract class BaseAreacliente extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'AreaclientePeer';
	 ...
	 * @return     Areacliente The current object (for fluent API support)
	 */
	public function setClienteId($v)
	{
		...
		if ($this->cliente_id !== $v) {
			$this->cliente_id = $v;
			$this->modifiedColumns[] = AreaclientePeer::CLIENTE_ID;
		}
	...
	public function setAreaId($v)
	{
		...
		if ($this->area_id !== $v) {
			$this->area_id = $v;
			$this->modifiedColumns[] = AreaclientePeer::AREA_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = AreaclientePeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating Areacliente object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = AreaclientePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				AreaclientePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedRows += AreaclientePeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = AreaclientePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = AreaclientePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['Areacliente'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Areacliente'][serialize($this->getPrimaryKey())] = true;
		$keys = AreaclientePeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AreaclientePeer::CLIENTE_ID)) $criteria->add(AreaclientePeer::CLIENTE_ID, $this->cliente_id);
		if ($this->isColumnModified(AreaclientePeer::AREA_ID)) $criteria->add(AreaclientePeer::AREA_ID, $this->area_id);
	...
	{
		$criteria = new Criteria(AreaclientePeer::DATABASE_NAME);
		$criteria->add(AreaclientePeer::CLIENTE_ID, $this->cliente_id);
		$criteria->add(AreaclientePeer::AREA_ID, $this->area_id);
	...
	{
		return (null === $this->getClienteId()) && (null === $this->getAreaId());
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Areacliente (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Areacliente Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     AreaclientePeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new AreaclientePeer();
		}
		return self::$peer;
	}
	...
	public function getCliente(PropelPDO $con = null)
	{
		if ($this->aCliente === null && ($this->cliente_id !== null)) {
			...
			/* The following can be used additionally to
				...
				$this->aCliente->addAreaclientes($this);
			 */
		}
	...
	public function getArea(PropelPDO $con = null)
	{
		if ($this->aArea === null && ($this->area_id !== null)) {
			...
			/* The following can be used additionally to
				...
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aArea->addAreaclientes($this);
			 */
		}
		return $this->aArea;
	}
	...
	{
		return (string) $this->exportTo(AreaclientePeer::DEFAULT_STRING_FORMAT);
	}
